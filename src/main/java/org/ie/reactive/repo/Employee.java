...
public class Employee {
	...
	public Uni<Boolean> save(MySQLPool client) {
		return client.preparedQuery("INSERT INTO employee(userId, role) VALUES (?, ?)").execute(Tuple.of(userId, role))
				.onItem().transform(pgRowSet -> pgRowSet.rowCount() == 1);
	}

	public static Uni<Boolean> delete(MySQLPool client, Integer userId) {
		return client.preparedQuery("DELETE FROM employee WHERE userId = ?").execute(Tuple.of(userId)).onItem()
